...
function Index({ setTab }) {
    ...
    const calculateTotalIncome = (items) => {
      ...
      useEffect(() => {
        ...
        const totalIncome = calculateTotalIncome(filteredItems);
    ...
    const handleFilter = () => {
        ...
        newArray = newArray.filter(item => item.status.toLowerCase().includes(status.toLowerCase()))
        if (dateFrom && dateTo) {
    ...
    const [visibleItems, setVisibleItems] = useState(new Array(handleFilter().length).fill(true));
    const popup = (index) => {
        swal({
            title: "Are you sure?",
          ...
          .then((willDelete) => {
            if (willDelete) {
                // Create a copy of the current listIncome
             const updatedListIncome = [...listIncome];
      
             // Remove the item at the specified index
             updatedListIncome.splice(index, 1);

             // Update the state with the new array
            setFilters(updatedListIncome);
    ...
    const fetchMoreData = async () => {
        try {
          ...
          // Mock new data
          const newData = [
            { name: "New Item 1", date: "2023-08-01", price: 15000000, category: "Bonus" },
            { name: "New Item 2", date: "2023-08-02", price: 8000000, category: "Gift" },
        ...
        <Layout tab={'income'} setTab={setTab}>
            <div className='w-full  p-10 max-w-[1200px]'>
                <div className='text-left text-3xl font-bold'>
                    Tổng thu nhập | <span className="text-green-500">+ {calculateTotalIncome(handleFilter())} VND</span>
                </div>
                <div className='grid grid-cols-3 py-10 gap-10'>
                    ...
                    <div className="col-span-2 py-10">
                        <div className="flex items-center justify-between gap-4 mb-4 grid grid-cols-3">
                            <input type="text" placeholder="search by name" className="border-[1px] border-gray-200 p-2 outline-none  col-span-1" onChange={e => setName(e.target.value)}/>
                            
                            <select name="category" id="" className="border-[1px] border-gray-200 p-2 outline-none col-span-1" onChange={e => setCategory(e.target.value)}>
                                <option value="Quà tặng sinh nhật">Quà tặng sinh nhật</option>
                                <option value="Chuyển khoản từ người thân">Chuyển khoản từ người thân</option>
                                <option value="Tiền thưởng cuối năm">Tiền thưởng cuối năm</option>
                                <option value="Hoa hồng dự án A">Hoa hồng dự án A</option>
                                <option value="Lương cứng">Lương cứng</option>
                            </select>
                            ...
                            <div className="col-span-1">
                              <label>From:</label>
                              <input type="date" className="border-[1px] border-gray-200 p-2 outline-none " onChange={e => setDateFrom(e.target.value)} />
                            </div>
                            <div className="col-span-1">
                              <label>To:</label>
                              <input type="date" className="border-[1px] border-gray-200 p-2 outline-none" onChange={e => setDateTo(e.target.value)} />
                            </div>
                                ...
                                <div key={index} className="w-full p-3 mt-5 bg-slate-100 flex items-center border-[1px] border-gray-200 justify-start rounded-lg">
                                    ...
                                    <div className="flex-1 flex flex-col items-start">
                                        <div className="flex items-center">
                                            <div className={filter.status === "income" ? "text-green-400" : "text-red-400" }><GoDotFill /></div>
                                            <div>{filter.name}</div>
                                        </div>
                                        <div className="flex items-center">
                                    ...
                                    <div className="text-3xl flex items-center">
                                        ...
                                        <span onClick={() => popup(index)}  className="text-red-500 px-2">
                                            <TiDelete />
                                        </span>
                                    </div>
                                </div>
